...
export default function AdminDashboard() {
  ...
  const handleUploadSlip = async (formData) => {
    try {
      const adminToken = localStorage.getItem('adminToken')
      const response = await fetch('/api/admin/upload-slip', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${adminToken}`
        },
  ...
  return (
    <>
      <Head>
        <title>Admin Dashboard - GAMCA Medical Verification</title>
        <meta name="description" content="Admin dashboard for managing medical appointments" />
