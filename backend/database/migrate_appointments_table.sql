...
SET @create_sql = IF(@table_exists = 0,
    ...
    'SELECT "Appointments table already exists" as message'
);
...
-- Check if appointment_id column exists in payment_transactions table
SET @appointment_column_exists = (
    SELECT COUNT(*) 
...
-- Update payment_transactions table to reference appointments only if column doesn't exist
SET @alter_sql = IF(@appointment_column_exists = 0 AND @table_exists = 0,
    ...
    'SELECT "Payment transactions table already has appointment_id column" as message'
);
...
SELECT 'Appointments table migration completed successfully' as result;